...
普通用户优先下载并运行 GitHub Actions 生成的 `LooyWindowsController-Setup-0.7.4.exe`，不需要执行下面的源码构建脚本。安装程序默认安装到当前用户目录，不要求管理员权限。
...
构建结束后，在该次 Actions 页面下载 `LooyWindowsController-Windows-v0.7.4` artifact 即可。
