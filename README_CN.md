...
普通用户优先下载并运行 GitHub Actions 生成的 `LooyWindowsController-Setup-0.7.3.exe`，不需要执行下面的源码构建脚本。安装程序默认安装到当前用户目录，不要求管理员权限。
...
构建结束后，在该次 Actions 页面下载 `LooyWindowsController-Windows-v0.7.3` artifact 即可。
...
0.7.1 修复网易云、抖音和 QQ 在“搜索”占位文字不存在或被旧关键词替换时无法定位的问题。程序不再点击 OCR 识别到的“搜索”文字来猜测输入框，而是根据已确认的前台应用和窗口尺寸生成安全候选输入区，始终使用一次短单击聚焦，输入后只在同一候选区核对；核对成功后才按画面选择搜索按钮或回车，失败则撤销。抖音网页候选点会主动避开“搜索”按钮，减少误点和整页全选。

0.7.2 将设备授权切换到独立的 `workers.dev` 公网网关，修复其他电脑使用后台激活码时直接“请求失败”的问题，并增加状态页和具体网络错误提示。后台仍可按设备单独启用、封禁、设置到期时间或永久授权；联网设备会在应用启动或下一次心跳（最多约 15 分钟）应用状态，当前离线宽限最长为 72 小时。

0.7.3 在激活窗口内置完整《用户协议、隐私与敏感权限说明》，即使在线页面暂时不可用也能阅读；协议同意和屏幕识别/键盘鼠标/MCP 数据流向特别授权分开勾选，均不预选。旧设备升级后不会由客户端自动代表用户接受新版条款，必须重新阅读确认；已绑定设备重新同意时无需再次输入激活码。
