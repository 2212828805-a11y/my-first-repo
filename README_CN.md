...
普通用户优先下载并运行 GitHub Actions 生成的 `LooyWindowsController-Setup-0.7.5.exe`，不需要执行下面的源码构建脚本。安装程序默认安装到当前用户目录，不要求管理员权限。
...
构建结束后，在该次 Actions 页面下载 `LooyWindowsController-Windows-v0.7.5` artifact 即可。
...
0.7.4 增加严格在线授权：客户端每 5 秒复核设备状态，管理员在后台确认封禁后，保持联网的应用通常会在约 5 秒内停止连接并关闭；取消 72 小时离线宽限，无法完成联网校验时应用会停用。服务器对高频校验只读取最新授权状态，并将设备在线记录限频写入，避免高频心跳造成不必要的数据库写入。已断网设备无法接收即时状态，恢复联网后会在首次校验时停用。
